...
### 5.1 F1: 掃碼維修頁面 (`ScanRepair.cshtml`)
...
### 5.4 F5 & F6: 儲存與主頁入口
* **入口**：在 `MaterialGunRepairView.cshtml` 的上方工具列加入 `[ 掃碼維修 ]` 按鈕，連結至 `ScanRepair`。
* **儲存**：在 `ScanRepair.cshtml` 點擊儲存，呼叫 `POST /MaterialGun/ScanRepairSave`，成功後 `Swal.fire` 提示並於 1.5 秒後跳轉回 `MaterialGunRepairView` 主頁。
...
sequenceDiagram
    ...
    participant V as ScanRepair (Vue 3)
